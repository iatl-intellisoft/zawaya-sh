...
class FinanceApproval(models.Model):
    ...
    attachment_ids = fields.Binary(string="Attachments",
                                      help="Copy of the ministry contract")
    
    # archived = fields.Boolean(string='Archived', default=False, readonly=True, copy=False)
    request_ids = fields.One2many(comodel_name="approval.type", inverse_name="request_id")
    approval_id = fields.Many2one(comodel_name="approval.type", string="Reqeust Type", required=False,)
    ...
    state = fields.Selection([('draft', 'Request'),
                              ...
                              ('cashier_conf', 'Cashier Confirmation'),
                              ('reject', 'Rejected'),
                              ('validate', 'Validated')],
    ...
    credit_partner_id = fields.Many2one('res.partner', 'Partner')
    # Finance Approval
    ...
    def get_approved_amount(self):
        for rec in self:
            rec.approved_amount = rec.request_amount
    ...
    def action_submit(self):
        state = 'dir_app'
        self.state = state
        if self.approval_id.need_attachment and not self.attachment_ids:
            raise ValidationError(_("Please add an attachment !"))
        return True
    ...
    def department_manager_approve(self):
        state = 'gm_app'
        self.gm_id = self.env.user.id
        self.state = state
        return True
        

    def finance_manger_approval(self):
        # state = 'au_app'
        state = 'cashier_conf'
    ...
    def validate(self):
        ...
        if self.on_credit:
            ...
            credit.append((0, 0, {
                ...
                'partner_id':self.credit_partner_id.id,
            }))
        ...
        if not self.on_credit:
            if not self.finance_approval_line_ids:
                raise UserError('Please Add Finance Approval Details ')
...
class ApprovalType(models.Model):
    ...
    request_id = fields.Many2one('finance.approval')
