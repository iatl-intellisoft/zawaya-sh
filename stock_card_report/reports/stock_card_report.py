...
class StockCardView(models.TransientModel):
    ...
    lot_id = fields.Many2one('stock.production.lot',string='Batch')
...
class StockCardReport(models.TransientModel):
    ...
    def _compute_results(self):
        ...
        self._cr.execute(
            ...
            WHERE (move.location_id in %s or move.location_dest_id in %s)
                and move.state in ('done') and move.product_id in %s
                and CAST(move.date AS date) <= %s
            ORDER BY move.date, move.reference
        """,
            (
    ...
    def _get_closing(self , result):
        close = 0
        product_balance = 0
        for rec in result:
            product_balance += rec.product_in - rec.product_out
            close = product_balance
        return close
