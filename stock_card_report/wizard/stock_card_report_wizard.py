...
class StockCardReportWizard(models.TransientModel):
    ...
    product_cat_ids = fields.Many2many('product.category', string='Category')
    ...
    location_id = fields.Many2one(
        comodel_name="stock.location", string="الموقع", required=True
    )
    # location_id = fields.Many2one(comodel_name="stock.location",domain = lambda self: [('id', 'in', self.env.user.stock_location_ids.ids)])

    product_ids = fields.Many2many(
        comodel_name="product.product", string="الصنف", required=True
    )
