...
class StockRule(models.Model):
    ...
    activity_mail_id = fields.Many2one('mail.activity', 'Activity')
        ...
        for domain, procurements_rules in procurements_by_po_domain.items():
            ...
            if not po:
                ...
                if positive_values:
                    ...
                    for rec in po:
                        # self.activity_mail_ids.unlink()
                        users = self.env['res.groups'].sudo().search([('id', '=',
                                                                       self.env.ref(
                                                                           'purchase.group_purchase_manager').ids)]).users
                        for user in users:
                            vals = {
                                ...
                                'user_id': user.id,
                                'summary': 'this RFQ create base on reordering rule' + ' ' + str(po.name),
                            }
                            self.activity_mail_id = self.env['mail.activity'].sudo().create(vals)
            else:
            ...
            for rec in po:
                ...
                for user in users:
                    vals = {
                        ...
                        'res_model_id': self.env.ref('purchase.model_purchase_order').id,
                        'user_id': user.id,
                        'summary': 'some products add to this RFQ base on reordering rule' + ' ' + str(po.name),
                    }
                    self.activity_mail_id = self.env['mail.activity'].sudo().create(vals)
