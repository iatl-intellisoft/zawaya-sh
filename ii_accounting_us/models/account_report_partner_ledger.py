...
class AccountPartnerLedgerReport(models.AbstractModel):
    _name = "account.partner.ledger.usd"
    ...
    def _get_templates(self):
        templates = super(AccountPartnerLedgerReport, self)._get_templates()
    ...
    def _get_options_domain(self, options):
        ...
        domain = super(AccountPartnerLedgerReport, self)._get_options_domain(options)
        ...
        if exch_code:
            domain += ['!', '&', '&', '&', ('us_credit', '=', 0.0), ('us_debit', '=', 0.0), ('us_amount_currency', '!=', 0.0), ('journal_id.id', 'in', exch_code.ids)]
    ...
    def _get_query_sums(self, options, expanded_partner=None):
        ...
        queries.append('''
            SELECT
                ...
                'sum'                               AS key,
                SUM(ROUND(account_move_line.us_debit))   AS us_debit,
                SUM(ROUND(account_move_line.us_credit))  AS us_credit,
                SUM(ROUND(account_move_line.us_balance)) AS us_balance
            FROM %s
    ...
    def _get_lines_without_partner(self, options, expanded_partner=None, offset=0, limit=0):
        ...
        query = '''
            SELECT
                ...
                account_move_line.us_amount_currency,
                account_move_line.matching_number,
                CASE WHEN aml_with_partner.us_balance > 0 THEN 0 ELSE partial.amount END AS us_debit,
                CASE WHEN aml_with_partner.us_balance < 0 THEN 0 ELSE partial.amount END AS us_credit,
                CASE WHEN aml_with_partner.us_balance > 0 THEN -partial.amount ELSE partial.amount END AS us_balance,
    ...
    def _get_sums_without_partner(self, options, expanded_partner=None):
        ...
        query = '''
            SELECT
                ...
                SUM(CASE WHEN aml_with_partner.us_balance > 0 THEN 0 ELSE partial.amount END) AS us_debit,
                SUM(CASE WHEN aml_with_partner.us_balance < 0 THEN 0 ELSE partial.amount END) AS us_credit,
                SUM(CASE WHEN aml_with_partner.us_balance > 0 THEN -partial.amount ELSE partial.amount END) AS us_balance,
    ...
    def _get_query_amls(self, options, expanded_partner=None, offset=None, limit=None):
        ...
        query = '''
            SELECT
                ...
                account_move_line.us_amount_currency,
                account_move_line.matching_number,
                ROUND(account_move_line.us_debit)   AS us_debit,
                ROUND(account_move_line.us_credit)  AS us_credit,
                ROUND(account_move_line.us_balance) AS us_balance,
                account_move_line__move_id.name         AS move_name,
    ...
    def _do_query(self, options, expanded_partner=None):
        ...
        def assign_sum(row):
            key = row['key']
            fields = ['us_balance', 'us_debit', 'us_credit'] if key == 'sum' else ['us_balance']
        ...
        if expanded_partner or unfold_all or options['unfolded_lines']:
            ...
            for row in self._cr.dictfetchall():
                ...
                if None in groupby_partners:
                    ...
                    none_row['us_debit'] = row['us_credit']
                    none_row['us_credit'] = row['us_debit']
                    none_row['us_balance'] = -row['us_balance']
        ...
        for row in self._cr.dictfetchall():
            key = row['key']
            total_debit += key == 'sum' and row['us_debit'] or 0
            total_credit += key == 'sum' and row['us_credit'] or 0
            total_initial_balance += key == 'initial_balance' and row['us_balance'] or 0
            total += key == 'sum' and row['us_balance'] or 0
        ...
        if None in groupby_partners:
            if 'sum' not in groupby_partners[None]:
                groupby_partners[None].setdefault('sum', {'us_debit': 0, 'us_credit': 0, 'us_balance': 0})
            if 'initial_balance' not in groupby_partners[None]:
                groupby_partners[None].setdefault('initial_balance', {'us_balance': 0})
            #debit/credit are inversed for the unknown partner as the computation is made regarding the balance of the known partner
            groupby_partners[None]['sum']['us_debit'] += total_credit
            groupby_partners[None]['sum']['us_credit'] += total_debit
            groupby_partners[None]['sum']['us_balance'] -= total
            groupby_partners[None]['initial_balance']['us_balance'] -= total_initial_balance
    ...
    @api.model
    def _get_report_line_partner(self, options, partner, initial_balance, us_debit, us_credit, us_balance):
        company_currency = self.env.company.currency_id
        unfold_all = self._context.get('print_mode') and not options.get('unfolded_lines')
        currency_usd = self.env['res.currency'].search([('name', '=', 'USD')], limit=1)
        columns = [
            {'name': self.format_value(initial_balance,currency=currency_usd), 'class': 'number'},
            {'name': self.format_value(us_debit,currency=currency_usd), 'class': 'number'},
            {'name': self.format_value(us_credit,currency=currency_usd), 'class': 'number'},
        ]
        if self.user_has_groups('base.group_multi_currency'):
            columns.append({'name': ''})
        columns.append({'name': self.format_value(us_balance,currency=currency_usd), 'class': 'number'})
        ...
        return {
            ...
            'unfoldable': not company_currency.is_zero(us_debit) or not company_currency.is_zero(us_credit),
    ...
    def _get_report_line_move_line(self, options, partner, aml, cumulated_init_balance, cumulated_balance):
        ...
        columns = [
            ...
            {'name': self.format_value(cumulated_init_balance,currency=currency_usd), 'class': 'number'},
            {'name': self.format_value(aml['us_debit'],currency=currency_usd ,blank_if_zero=True), 'class': 'number'},
            {'name': self.format_value(aml['us_credit'],currency=currency_usd,blank_if_zero=True), 'class': 'number'},
        ]
        if self.user_has_groups('base.group_multi_currency'):
            if aml['currency_id']:
                currency = self.env['res.currency'].browse(aml['currency_id'])
                print("___________________________",currency)
                formatted_amount = self.format_value(aml['us_amount_currency'], currency=currency_usd, blank_if_zero=True)
                print("++++++++++++++++++++++++++=",formatted_amount)
                columns.append({'name': formatted_amount, 'class': 'number'})

            else:
                columns.append({'name': ''})
        columns.append({'name': self.format_value(cumulated_balance,currency=currency_usd), 'class': 'number'})
        return {
            'id': aml['id'],
    ...
    @api.model
    def _get_report_line_total(self, options, initial_balance, us_debit, us_credit, us_balance):
        currency_usd = self.env['res.currency'].search([('name', '=', 'USD')], limit=1)
        columns = [
            {'name': self.format_value(initial_balance,currency=currency_usd), 'class': 'number'},
            {'name': self.format_value(us_debit,currency=currency_usd), 'class': 'number'},
    ...
    def _get_partner_ledger_lines(self, options, line_id=None):
        ...
        for partner, results in partners_results:
            ...
            initial_balance = partner_init_bal.get('us_balance', 0.0)
            debit = partner_sum.get('us_debit', 0.0)
            credit = partner_sum.get('us_credit', 0.0)
            balance = initial_balance + partner_sum.get('us_balance', 0.0)
            ...
            if unfold_all or is_unfolded:
                ...
                for aml in amls:
                    ...
                    cumulated_balance += aml['us_balance']
    ...
    def _load_more_lines(self, options, line_id, offset, load_more_remaining, progress):
        ...
        for row in self._cr.dictfetchall():
            ...
            if line_id == 'loadmore_0':
                # reconciled lines without partners are fetched to be displayed under the matched partner
                # and thus but be inversed to be displayed under the unknown partner
                row['us_debit'] = row['us_balance'] < 0 and row['us_credit'] or 0
                row['us_credit'] = row['us_balance'] > 0 and row['us_debit'] or 0
                row['us_balance'] = -row['us_balance']
            cumulated_init_balance = progress
            progress += row['us_balance']
    ...
    def _get_columns_name(self, options):
        columns = [
            ...
            {'name': _('US Debit'), 'class': 'number'},
            {'name': _('US Credit'), 'class': 'number'}]

        if self.user_has_groups('base.group_multi_currency'):
            columns.append({'name': _('US Amount Currency'), 'class': 'number'})

        columns.append({'name': _('US Balance'), 'class': 'number'})
