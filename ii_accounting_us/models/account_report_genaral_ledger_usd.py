...
class AccountGeneralLedgerReport(models.AbstractModel):
    _name = "account.general.ledger.usd"
    ...
    def _get_columns_name(self, options):
        columns_names = [
            ...
            {'name': _('Partner')},
            {'name': _('USD Debit'), 'class': 'number'},
            {'name': _('USD Credit'), 'class': 'number'},
            {'name': _('USD Balance'), 'class': 'number'}
        ]
    ...
    def _get_general_ledger_lines(self, options, line_id=None):
        ...
        for account, periods_results in accounts_results:
            ...
            us_amount_currency = account_sum.get('us_amount_currency', 0.0) + account_un_earn.get('us_amount_currency', 0.0)
            us_debit = account_sum.get('us_debit', 0.0) + account_un_earn.get('us_debit', 0.0)
            us_credit = account_sum.get('us_credit', 0.0) + account_un_earn.get('us_credit', 0.0)
            us_balance = account_sum.get('us_balance', 0.0) + account_un_earn.get('us_balance', 0.0)
            lines.append(self._get_account_title_line(options, account,us_amount_currency, us_debit, us_credit, us_balance, has_lines))
            total_debit += us_debit
            total_credit += us_credit
            total_balance += us_balance

            if has_lines and (unfold_all or is_unfolded):
                # Initial balance line.
                account_init_bal = results.get('initial_balance', {})

                cumulated_balance = account_init_bal.get('us_balance', 0.0) + account_un_earn.get('us_balance', 0.0)

                lines.append(self._get_initial_balance_line(
                    options, account,
                    account_init_bal.get('us_amount_currency', 0.0) + account_un_earn.get('us_amount_currency', 0.0),
                    account_init_bal.get('us_debit', 0.0) + account_un_earn.get('us_debit', 0.0),
                    account_init_bal.get('us_credit', 0.0) + account_un_earn.get('us_credit', 0.0),
                    cumulated_balance,
                ))
                ...
                if self.env.company.totals_below_sections:
                    ...
                    lines.append(self._get_account_total_line(
                        options, account,
                        account_sum.get('us_amount_currency', 0.0),
                        account_sum.get('us_debit', 0.0),
                        account_sum.get('us_credit', 0.0),
                        account_sum.get('us_balance', 0.0),
                    ))
    ...
    def _load_more_lines(self, options, line_id, offset, load_more_remaining, balance_progress):
        ...
        for aml in self._cr.dictfetchall():
            ...
            balance_progress += aml['us_balance']
        ...
        for i, options_period in enumerate(options_list):
            ...
            queries.append('''
                SELECT
                    ...
                    %s                                                      AS period_number,
                    COALESCE(SUM(account_move_line.us_amount_currency), 0.0)   AS us_amount_currency,
            ...
            for i, options_period in enumerate(options_list):
                ...
                queries += ['''
                    SELECT
                        ...
                        NULL                                    AS max_date,
                        %s                                      AS period_number,
                        0.0                                     AS us_amount_currency,
                        0.0                                     AS debit,
                        0.0                                     AS credit,
                        SUM(ROUND(account_move_line.us_balance * currency_table.rate, currency_table.precision)) AS us_balance
    ...
    def _get_query_amls(self, options, expanded_account, offset=None, limit=None):
        ...
        query = '''
            SELECT
                ...
                account_move_line.currency_usd,
    ...
    def _do_query(self, options_list, expanded_account=None, fetch_lines=True):
        ...
        for res in self._cr.dictfetchall():
            ...
            if key == 'sum':
                ...
                groupby_taxes[res['groupby']][key] = res['us_balance']
    ...
    @api.model
    def _get_account_title_line(self, options, account,us_amount_currency, us_debit, us_credit, us_balance, has_lines):
        ...
        columns = [
            ...
            {'name': self.format_value(us_balance,currency=currency_usd), 'class': 'number'},
        ]
        if self.user_has_groups('base.group_multi_currency'):
            columns.insert(0, {'name': has_foreign_currency and self.format_value(us_amount_currency, currency=currency_usd, blank_if_zero=True) or '', 'class': 'number'})
        return {
    ...
    @api.model
    def _get_initial_balance_line(self, options, account, us_amount_currency, us_debit, us_credit, us_balance):
        currency_usd = self.env['res.currency'].search([('name', '=', 'USD')], limit=1)
        columns = [
            {'name': self.format_value(us_debit,currency=currency_usd), 'class': 'number'},
            {'name': self.format_value(us_credit,currency=currency_usd), 'class': 'number'},
        ...
        if self.user_has_groups('base.group_multi_currency'):
            columns.insert(0, {'name': self.format_value(us_amount_currency, currency=currency_usd, blank_if_zero=True), 'class': 'number'})
        return {
    ...
    def _get_aml_line(self, options, account, aml, cumulated_balance):
        ...
        columns = [
            ...
            {'name': self.format_value(cumulated_balance,currency= currency_usd), 'class': 'number'},
        ]
        if self.user_has_groups('base.group_multi_currency'):
            columns.insert(3, {'name': currency and aml['us_amount_currency'] and self.format_value(aml['us_amount_currency']*rate, currency=currency_usd, blank_if_zero=True) or '', 'class': 'number'})
        return {
            'id': aml['id'],
    ...
    @api.model
    def _get_account_total_line(self, options, account, us_amount_currency,us_debit, us_credit, us_balance):
        has_foreign_currency = account.currency_id and account.currency_id != account.company_id.currency_id or False
    ...
    @api.model
    def _get_total_line(self, options, us_debit, us_credit, us_balance):
        currency_usd = self.env['res.currency'].search([('name', '=', 'USD')], limit=1)
        return {
