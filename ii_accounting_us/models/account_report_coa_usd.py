...
class AccountChartOfAccountReportUS(models.AbstractModel):
    _name = "account.coa.usd.report"
    ...
    def _get_templates(self):
        templates = super(AccountChartOfAccountReportUS, self)._get_templates()
        # self.name = "Trial Balance USD"
    ...
    def _get_columns(self, options):
        ...
        header2 = [
            {'name': '', 'style': 'width:40%'},
            {'name': _('USD Debit'), 'class': 'number o_account_coa_column_contrast'},
            {'name': _('USD Credit'), 'class': 'number o_account_coa_column_contrast'},
        ]
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for account, periods_results in accounts_results:
            ...
            for i, period_values in enumerate(reversed(periods_results)):
                ...
                if i == 0:
                    ...
                    initial_balance = account_init_bal.get('us_balance', 0.0) + account_un_earn.get('us_balance', 0.0)
                    sums += [
                ...
                sums += [
                    account_sum.get('us_debit', 0.0) - account_init_bal.get('us_debit', 0.0),
                    account_sum.get('us_credit', 0.0) - account_init_bal.get('us_credit', 0.0),
                ]
            ...
            for i, value in enumerate(sums):
                ...
                # Create columns.
                columns.append({'name': self.format_value(value,currency=currency_usd, blank_if_zero=True), 'class': 'number', 'no_format_name': value})
        ...
        lines.append({
             ...
             'columns': [{'name': self.format_value(total,currency=currency_usd), 'class': 'number'} for total in totals],
             'level': 1,
        })
